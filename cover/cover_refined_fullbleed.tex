...
% Updated with 0.8" bleed for Mixam requirements
% Base cover dimensions: 7in x 10in (each cover) + 1.29in spine = 15.29in x 10in
% Total trim size: 15.66in x 10in
% With 0.8" bleed on all sides: 17.26in x 11.6in
\usepackage[paperwidth=17.26in,paperheight=11.6in,margin=0in]{geometry}
\usepackage{graphicx}
\usepackage{tikz}
\usepackage{xcolor}
...
\begin{document}
\null % Initialize the page
\begin{tikzpicture}[remember picture,overlay]
  % Place background image centered on page - scaled to include 0.8" bleed
  \node[anchor=center, inner sep=0pt] at (current page.center) {
    \includegraphics[width=17.26in,height=11.6in,keepaspectratio=false]{cover_wrap_hires.png}
  };
  ...
  \node[anchor=center] at ([xshift=\frontoffset, yshift=2.8in]current page.center) {
    ...
    \node[anchor=center] at ([xshift=\frontoffset, yshift=-0.2in]current page.center) {
      \begin{tcolorbox}[draftbox, width=4in]
        ...
        DRAFT VER 100 NOV 14 2025\\[0.2em]
        \fontsize{11}{13}\selectfont
        Includes only 15 factual errors!
      \end{tcolorbox}
    };
